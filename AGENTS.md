...
| Venda inteligente no carrinho (`Complete seu mix`) | `sugerirProdutosCarrinho` em `api/services/loja.ts` + `POST /api/loja/sugestoes-carrinho`; UI no drawer do carrinho em `src/loja/Loja.tsx` |
...
- **Sugestões do carrinho são estatísticas, não IA generativa**: `POST /loja/sugestoes-carrinho` cruza os itens do carrinho com os produtos mais vendidos para clientes parecidos. Escopo em cascata: mesmo `segmento+cidade/UF` → mesmo `segmento+UF` → mesmo `segmento` → mesma `cidade/UF` → global WIN. Remove itens já no carrinho e produtos do mesmo `grupo_chave` para não sugerir outro sabor do produto escolhido.
